...
\begin{document}
	...
	Il BNO055, inoltre, supporta tre differenti modalità di alimentazione: \textit{Normal mode}, \textit{Low power mode} e \textit{Suspend mode}. Per lo sviluppo è stata scelta la \textbf{\textit{Normal mode}}.
	
	\paragraph{\small{Protocollo di comunicazione}}\mbox{}\\
	Il \textit{BNO055} offre la possibilità di utilizzare due distinti protocolli di comunicazione seriale, \textit{$I^{2}C$} e \textit{UART}. Nel presente progetto è stato utilizzato esclusivamente il protocollo \textit{$I^{2}C$}.
	...
	Prima di descrivere le caratteristiche dell'accelerometro, del giroscopio e del magnetometro installati nel \textit{BNO055}, si ritiene necessario dedicare una sezione di approfondimento ai principi di funzionamento dei sensori appena citati.
	\paragraph{Accelerometro}\mbox{}\\
	L'accelerometro integrato nel BNO055 Bosch Sensortec è di tipo capacitivo M.E.M.S, \textit{Micro-Electro-Mechanical System}. Il dispositivo consente la misura dell'accelerazione lungo i tre assi cartesiani.\\
	...
	\paragraph{Accelerometro}\mbox{}\\
	L'accelerometro installato sul \textit{BNO055} appartiene alla famiglia \textit{Bosch Sensortec}. La sua \textbf{risoluzione} è di \textbf{14 bit}.
	...
	e velivoli, si impiegano filtri come \textit{EKF} (Filtro di Kalman esteso) o \textit{UKF} (Filtro di Kalman a Punti Sigma). La documentazione ufficiale del BNO055 riporta che il microcontrollore presenta un
