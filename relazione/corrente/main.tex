...
\begin{document}
	...
	\subsection{Diagramma di flusso del fimware}
	\begin{figure}[H]
		\centering
	...
	Il firmware è organizzato secondo una struttura modulare in cui ciascun file incapsula una responsabilità ben definita. Il file \texttt{main.c} svolge il ruolo di supervisore dell'intero sistema: inizializza le periferiche, gestisce la sequenza di avvio sicuro, configura i controllori e coordina il ciclo principale evento-driven. Le funzionalità specialistiche sono invece delegate a moduli separati: \texttt{pwm.c} contiene le primitive di attuazione per motori e servomotori, \texttt{pid.c} implementa il controllore PID generico e il filtro mediano usato sulla misura di quota, \texttt{tof.c} esegue la compensazione geometrica della misura del sensore ToF rispetto all'inclinazione del velivolo, mentre \texttt{imu.c} raccoglie le funzioni di lettura e riorientamento delle misure provenienti dal BNO055.
	
	Questa suddivisione non ha una sola finalità organizzativa, ma risponde direttamente a un'esigenza di affidabilità: le operazioni time-critical vengono mantenute semplici e localizzate, mentre la logica di controllo vera e propria resta concentrata nel ciclo principale. In questo modo le callback di interrupt non svolgono elaborazioni numeriche onerose, ma si limitano a notificare la disponibilità di un evento tramite le flag condivise \texttt{actuate\_motors\_control} e \texttt{actuate\_servo\_control}. La scelta riduce il tempo di permanenza in ISR, limita il jitter introdotto dalle interrupt e rende il comportamento del firmware più leggibile e deterministico.
	
	\subsection{Definizione delle costanti e delle strutture condivise}
	Il file \texttt{DPDF\_var\_def.h} raccoglie le costanti numeriche che definiscono i limiti operativi del sistema. Centralizzare questi valori in un unico header evita la dispersione di ``numeri magici'' nel codice e consente di distinguere con chiarezza ciò che dipende dalla logica di controllo da ciò che dipende dall'hardware reale, come l'escursione dei servo, i limiti accettabili per gli ESC e i parametri di temporizzazione della procedura di avvio.
	
	Le macro principali sono le seguenti:
	\begin{itemize}
		\item \texttt{NUMBER\_OF\_TOGGLES} stabilisce la durata del conto alla rovescia visivo prima dell'avvio;
		\item \texttt{CCR\_VALUE\_FOR\_MOTOR\_ACT}, \texttt{UPPER\_LIMIT\_MOTOR} e \texttt{LOWER\_LIMIT\_MOTOR} definiscono il range PWM utilizzabile per gli ESC;
		\item \texttt{UPPER\_LIMIT\_SERVO}, \texttt{CENTER\_SERVO} e \texttt{LOWER\_LIMIT\_SERVO} delimitano il campo di lavoro sicuro dei servomotori;
		\item \texttt{CCR\_PER\_DEGREE} e \texttt{MAX\_FLAP\_ANGLE\_DEG} fissano la conversione tra angolo richiesto al flap e valore da scrivere nei registri CCR.
	\end{itemize}
	La scelta di esprimere i vincoli hardware in termini di limiti espliciti ha un impatto diretto sulla sicurezza del controllo. I PID non producono comandi ``astratti'', ma valori che devono rimanere compatibili con i dispositivi fisici realmente montati sul DPDF. Inserire i limiti a livello di configurazione permette quindi di rendere il comportamento del firmware prevedibile e di evitare richieste non attuabili, che sui servo si tradurrebbero in fine corsa meccanici e sugli ESC in comandi fuori specifica.
	
	Il firmware utilizza inoltre una struttura PID unica, inizializzata tramite la funzione \texttt{pid\_init()} e aggiornata tramite \texttt{pid\_compute()}. La stessa implementazione viene impiegata sia per il controllo dei flap sia per il controllo della quota, cambiando soltanto i guadagni, il tempo di campionamento, i limiti di saturazione e alcune opzioni interne, come il calcolo della derivata sulla misura o sull'errore. Questa scelta riduce la duplicazione del codice e garantisce che tutte le catene di controllo seguano le stesse regole di inizializzazione, saturazione e aggiornamento dello stato interno.
	...
	Il ramo di controllo della quota è costruito attorno al sensore ToF \textit{VL53L1X}, usato come sorgente primaria della distanza dal piano di riferimento. La scelta di misurare direttamente la quota tramite un sensore a tempo di volo consente di chiudere l'anello su una grandezza fisicamente osservabile e immediatamente interpretabile, evitando di dedurla indirettamente da stime inerziali più sensibili a deriva e integrazione dell'errore.
	...
	Il dato fornito dal VL53L1X non viene utilizzato in modo diretto, ma attraversa una piccola catena di pre-elaborazione pensata per migliorare la robustezza del controllo. In primo luogo, la misura grezza viene filtrata mediante un filtro mediano a tre campioni. Questa scelta è particolarmente adatta al sensore ToF perché consente di attenuare spike impulsivi o misure isolate anomale senza introdurre il ritardo tipico di filtri medi più estesi. In secondo luogo, la distanza viene compensata rispetto all'inclinazione corrente del drone. Se il corpo del velivolo è inclinato, infatti, la misura lungo l'asse del sensore non coincide più con la quota verticale reale; per questo motivo il firmware applica una correzione geometrica proporzionale a \(\cos(roll)\cos(pitch)\), in modo da ricondurre il dato a una stima della componente verticale.
	
	Solo dopo queste due elaborazioni la quota stimata viene fornita al PID dei motori. Il controllore è inizializzato con un tempo di campionamento di \texttt{33 ms}, guadagno proporzionale pari a \texttt{3.0}, limiti di uscita compresi tra \texttt{LOWER\_LIMIT\_MOTOR} e \texttt{UPPER\_LIMIT\_MOTOR}, derivata calcolata sulla misura e termine derivativo filtrato con coefficiente \texttt{0.3}. Anche quando il guadagno derivativo è nullo, mantenere questa struttura rende il blocco di controllo già predisposto a successive fasi di taratura senza doverne modificare l'architettura. I due motori ricevono sempre lo stesso comando PWM, scritto in \texttt{TIM4->CCR2} e \texttt{TIM4->CCR3}, perché la regolazione della quota deve modificare la portanza totale senza introdurre volutamente momenti torcenti attorno all'asse verticale.
	
	Dal punto di vista esecutivo, il controllo della quota è attivato in modo asincrono dalla disponibilità di una nuova misura: quando il pin di \textit{data-ready} del VL53L1X genera l'interrupt, la callback non esegue alcun calcolo ma imposta solamente la flag \texttt{actuate\_motors\_control}. Nel ciclo principale il firmware può quindi processare l'evento con ordine ben definito: conferma e riarmo della misura del sensore, lettura del dato, filtraggio, compensazione dell'inclinazione, calcolo del comando e aggiornamento dell'attuazione. Questa separazione rende il tempo di risposta sufficientemente rapido senza compromettere la leggibilità del codice.
	...
	Il controllo di assetto impiega il sensore inerziale \textit{BNO055} in modalità \texttt{NDOF}, cioè nella configurazione in cui il dispositivo fornisce direttamente gli angoli di Eulero risultanti dalla fusione dei sensori interni. Questa modalità è adatta al DPDF perché riduce la complessità del firmware: il microcontrollore non deve implementare un osservatore d'assetto dedicato e può lavorare direttamente su grandezze già espresse in gradi, più immediate sia per la progettazione del controllore sia per il debug sperimentale.
	...
	L'informazione restituita dal BNO055 non viene però usata in maniera immediata per pilotare i flap. La geometria meccanica del sistema introduce infatti un disallineamento fisso tra il riferimento della IMU e il riferimento dei due flap di controllo. Per evitare che il PID lavori su assi non coerenti con gli attuatori, il firmware applica una trasformazione lineare che ruota roll e pitch nel sistema di riferimento dei flap. In altri termini, il controllo non corregge l'assetto nel riferimento del sensore, ma nel riferimento realmente osservato dagli attuatori aerodinamici. Questo passaggio è fondamentale perché rende la relazione tra errore d'assetto e azione correttiva molto più diretta e quindi più facilmente regolabile.
	
	I due controllori PID dei flap sono inizializzati con guadagno proporzionale pari a \texttt{4.0}, termini integrale e derivativo nulli, tempo di campionamento di \texttt{10 ms} e saturazione in uscita nell'intervallo \([-60^{\circ}, +60^{\circ}]\). La variabile di uscita del PID non è quindi ancora un valore PWM, ma un angolo richiesto per il flap. Questa scelta è concettualmente vantaggiosa perché mantiene separati il livello di controllo e il livello di attuazione: il controllore decide l'azione correttiva in grandezze fisiche, mentre il modulo PWM si occupa in un secondo momento di tradurre tale richiesta nel corrispondente valore di compare register.
	
	La conversione tra angolo desiderato e PWM è eseguita dalla funzione \texttt{angle\_to\_pwm()}, che usa il valore centrale del servo e il coefficiente \texttt{CCR\_PER\_DEGREE} per trasformare i gradi in conteggi timer, applicando poi un ulteriore \textit{clamp} ai limiti meccanicamente sicuri. In questo modo il firmware evita che una richiesta correttiva troppo grande si trasformi in uno sforzo inutile o potenzialmente dannoso per i servo.
	
	Anche il controllo di assetto è gestito secondo una logica event-driven. Il timer \texttt{TIM7} genera la base tempi periodica del ramo servo; la relativa callback imposta la flag \texttt{actuate\_servo\_control} e il ciclo principale, quando la rileva, esegue nell'ordine lettura IMU, remap degli assi, calcolo dei due PID, conversione in PWM e aggiornamento dei registri \texttt{TIM2->CCR1} e \texttt{TIM2->CCR2}. Il vantaggio di questa impostazione è duplice: il campionamento resta regolare e al tempo stesso il codice di controllo rimane fuori dal contesto di interrupt.
	...
	L'esecuzione del firmware segue una sequenza precisa che combina fasi iniziali strettamente ordinate e una successiva gestione concorrente degli eventi. Questa organizzazione è preferibile rispetto a una struttura interamente lineare perché separa nettamente le operazioni che devono avvenire una sola volta all'avvio dalle operazioni che devono invece ripetersi con frequenze diverse durante il funzionamento.
	
	La sequenza operativa può essere riassunta come segue:
	\begin{enumerate}
		\item \textbf{Safe startup}: il sistema attende un consenso esplicito all'avvio e utilizza \texttt{TIM6} per generare il conto alla rovescia visivo tramite LED, sfruttando \texttt{\_\_WFI()} nelle attese così da non occupare inutilmente la CPU;
		\item \textbf{Attivazione e inizializzazione sensori}: vengono alimentati i dispositivi esterni, eseguito il reset hardware del VL53L1X tramite \texttt{XSHUT} e configurato il BNO055 in modalità \texttt{NDOF} con il corretto remap degli assi;
		\item \textbf{Configurazione del controllo}: sono inizializzati i tre PID, il filtro mediano del sensore ToF e i canali PWM necessari per attuatori e motori;
		\item \textbf{Preparazione degli ESC}: i motori vengono portati inizialmente al valore minimo di comando, lasciando un intervallo di alcuni secondi per il corretto riconoscimento del segnale da parte degli ESC;
		\item \textbf{Avvio della fase operativa}: \texttt{TIM7} abilita il ramo periodico dei servo, mentre il VL53L1X avvia il flusso di misure asincrone che alimenta il ramo di quota;
		\item \textbf{Controllo nel main loop}: il \texttt{while(run)} riceve gli eventi dalle flag condivise, elabora i dati dei sensori e aggiorna gli attuatori senza effettuare busy-waiting o calcoli nelle ISR;
		\item \textbf{Shutdown sicuro}: quando \texttt{run} viene disattivata, il firmware riporta motori e servo in una condizione sicura tramite \texttt{stop\_all\_pwm()} e segnala visivamente la fine dell'esecuzione.
	\end{enumerate}
	
	L'intero modello esecutivo è costruito per mantenere semplice il percorso dei dati: le callback identificano l'evento, il \texttt{main} esegue l'elaborazione e i moduli dedicati applicano il comando. In questo modo ogni decisione di controllo può essere ricondotta a una sequenza chiara di passi causali, caratteristica importante sia in fase di validazione sperimentale sia in un'ottica di manutenzione futura del firmware.
	
	Per evidenziare questa scelta progettuale, il seguente frammento mostra come le interrupt si limitino a segnalare gli eventi al ciclo principale.
