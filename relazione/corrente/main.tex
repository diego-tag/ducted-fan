...
\begin{document}
	...
	L'obiettivo a lungo termine del progetto consiste nel raggiungimento di una condizione di \textit{hovering} stabile. All'interno di questo scenario, il \textit{task} assegnato al nostro gruppo prevede lo sviluppo dell'infrastruttura logica di controllo e l'avvio della fase di taratura sperimentale dei regolatori PID. Nel corso delle prove è stato riscontrato che la sola configurazione controrotante dei due motori non è sufficiente a compensare in modo affidabile le rotazioni attorno all'asse \textit{z}, per questo motivo una parte del lavoro si è progressivamente concentrata sull'introduzione e sulla taratura di un regolatore dedicato al controllo dello \textit{yaw}.\\
	Inoltre durante i primi approcci sperimentali è emerso come, prima ancora di poter ricercare i guadagni ottimali dei regolatori, il sistema necessitasse di fondamenta software ben più solide per poter operare in sicurezza. Di conseguenza, è stato necessario un profondo \textit{refactoring} e irrobustimento del \textit{firmware} ereditato. Per rendere il percorso \textit{misura--controllo--attuazione} affidabile e trasparente, abbiamo introdotto e implementato numerosi accorgimenti: un filtro mediano per la reiezione dei disturbi del sensore ToF, la compensazione geometrica del \textit{tilt}, il \textit{remap} matriciale delle misure inerziali, una gestione \textit{event-driven} dei loop di controllo, logiche sicure di saturazione e \textit{anti-windup} e, nella fase finale della sperimentazione, un PID dedicato alla stabilizzazione dello \textit{yaw}, divenuto il principale oggetto della taratura conclusiva.\\
	...
	Proseguendo con la descrizione della struttura interna del \textit{DPDF}, nella parte superiore sono posizionati due motori disposti in configurazione controrotante. In linea di principio, tale assetto dovrebbe consentire la generazione di spinte verticali limitando la rotazione del drone attorno all'asse Z, poiché le eliche producono momenti torcenti di verso opposto. Le prove sperimentali hanno tuttavia mostrato che questa compensazione passiva, pur utile, non è risultata sufficiente da sola a sopprimere in modo soddisfacente la dinamica di \textit{yaw}, rendendo necessaria l'introduzione di un'azione di controllo dedicata.\\
	...
	Per questa ragione il firmware adotta un'architettura a quattro regolatori PID: un anello dedicato alla quota, che modifica la portanza totale intervenendo sui motori; due anelli dedicati all'assetto, che generano l'angolo richiesto ai flap per contrastare gli errori di \textit{roll} e \textit{pitch}; e un ulteriore anello dedicato allo \textit{yaw}, introdotto quando è emerso sperimentalmente che la compensazione passiva offerta dai motori controrotanti non era sufficiente. La scelta dei PID non deriva da una preferenza generica per una soluzione nota, ma dal fatto che essi permettono di costruire un legame diretto e interpretabile tra errore misurato e azione correttiva. Nel caso specifico del nostro lavoro, il regolatore di \textit{yaw} ha assunto un ruolo centrale, diventando l'elemento su cui si è concentrata la fase finale di taratura sperimentale.
	...
	Il dispositivo dispone di circa \textbf{1 MByte} di SRAM, suddivisa in più banchi.
	...
	Lo STM32H745ZI-TQ6 integra tre \textbf{SAR ADC} a 16 bit: ADC1, ADC2 e ADC3. I primi due possono operare in modo accoppiato per conversioni sincronizzate, mentre la risoluzione è configurabile da 8 a 16 bit. Nel nostro progetto, tuttavia, tali periferiche non sono state impiegate, poiché l'acquisizione delle grandezze di interesse è avvenuta esclusivamente tramite sensori digitali.
	
	\paragraph{\small{Convertitori Digitali-Analogici}}\mbox{}\\
	Il microcontrollore dispone di due \textbf{DAC} indipendenti a 12 bit, che in questo lavoro non sono stati utilizzati, non essendo richiesta la generazione di segnali analogici.
	...
	\begin{itemize}
		...
		\item \texttt{TIM7}, per la gestione periodica della lettura in \textit{polling mode} dei dati dell'IMU.
	...
	Il Turnigy Plush 40A \cite{ESCDatasheet} è un controllore elettronico di velocità per motori \textit{brushless} BLDC. Il suo compito è convertire la tensione continua di alimentazione nelle tre fasi necessarie al pilotaggio del motore.
	...
	Il \textit{Turnigy Plush 40A} controlla i motori tramite un segnale PWM a $50$ [Hz], frequenza tipica delle applicazioni modellistiche. Prima del funzionamento è necessaria una procedura di armamento, ottenuta inviando un segnale con \textit{duty cycle} pari al $5\%$.
	...
	I \textit{flap} operano come superfici aerodinamiche di controllo dedicate alla compensazione dei disturbi di \textit{roll} e \textit{pitch}. Sono posizionati sulla parte terminale del condotto ed interagiscono con il getto generato
	...
	Per ottenere una rotazione oraria del rotore, il comando deve operare nel seguente intervallo di impulsi: \textbf{1.5$\div$2.1 [ms]}. L'effetto di rotazione antioraria si ha invece per: \textbf{0.9$\div$1.5 [ms]}.
